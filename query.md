...
- Expand `compiler/session` to own dense session-local ids for package, module, item, body, trait, impl, associated type, const, and reflection subject.
...
- Move trait declarations, impl declarations, associated-type bindings, callable signatures, reflectability flags, boundary flags, and const declarations into checked-signature facts.
...
- Support trait satisfaction, associated-type projection equality, impl `where` obligations, built-in marker-trait hooks, and default-method inheritance lookups.
...
- Query named const dependencies by `ConstId`; report cycles through the shared query stack.
- Use the same evaluator for module consts, local consts, array lengths, repetition lengths, and explicit enum discriminants.
- Delete the permanent direct `typed.Expr` CTFE path.
- Spec refs: `spec/semantic-query-and-checking.md`, `spec/consts.md`, `spec/arrays.md`, `spec/c-abi.md`.
...
- Spec refs: `spec/callables.md`, `spec/functions.md`, `spec/patterns.md`, `spec/send.md`, `spec/boundary-kinds.md`, `spec/boundary-contracts.md`, `spec/ownership-and-reflection.md`.
...
- **CTFE**
  - module consts, local consts, array lengths, enum discriminants
  - overflow, divide-by-zero, invalid shifts, invalid discriminants, and cycles
